...
def get_visitor_queue(society):
    ...
    return jsonify([{"name": v["name"], "flat": v["flat"], "position": i+1} for i, v in enumerate(q)])
...
def add_maintenance():
    ...
    push_notification(data.get("society",""), f"New maintenance bill added for Flat {data['flat']} — Rs.{data['amount']}")
...
def get_notifications(society):
    ...
    return jsonify(list(reversed(soc[-20:])))
...
def get_stats(society):
    return jsonify({
        "total_residents":  len(set(m["flat"] for m in maintenance if m.get("society") == society)),
...
def predict_peak(society):
    ...
    for a in soc_bookings:
        if a.get("start"):
            try:
                hour_counts[int(a["start"].split(":")[0])] += 1
            except: pass
        if a.get("date"):
            try:
...
def security_anomalies(society):
    ...
    for flat, count in flat_counts.items():
        if count > 3:
            anomalies.append({"type": "High Traffic", "icon": "🚨",
                "message": f"Flat {flat} has {count} visitors today - unusually high", "severity": "danger"})
        ...
        if v.get("time"):
            try:
                ...
                if hour >= 22 or hour < 6:
                    label = "late night" if hour >= 22 else "early morning"
                    anomalies.append({"type": "Odd Hours Entry", "icon": "🌙",
                        "message": f"Visitor '{v['name']}' at Flat {v['flat']} at {v['time']} ({label})", "severity": "warning"})
            except: pass
    ...
    for v in soc_visitors:
        if v.get("status") == "Inside" and v.get("entry_time"):
            try:
                hours_inside = now_hour - int(v["entry_time"].split(":")[0])
                if hours_inside > 4:
                    anomalies.append({"type": "Long Stay Alert", "icon": "⏰",
                        "message": f"Visitor '{v['name']}' at Flat {v['flat']} inside for {hours_inside}+ hours", "severity": "warning"})
            except: pass

    name_counts = Counter(v["name"].lower() for v in soc_visitors)
    for name, count in name_counts.items():
        if count > 1:
            anomalies.append({"type": "Duplicate Visitor", "icon": "👥",
                "message": f"Visitor '{name.title()}' appears {count} times - possible duplicate", "severity": "warning"})

    return jsonify({"anomalies": anomalies, "total": len(anomalies), "scanned": len(soc_visitors)})
